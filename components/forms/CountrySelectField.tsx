...
const CountrySelect = ({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          {value ? (
            <span className="flex items-center gap-2">
              <span>
                {
                  countriesWithFlag.find((country) => country.alpha2 === value)
                    ?.emoji
                }
              </span>
              <span>
      ...
      <PopoverContent
        className="w-full p-0 border-gray-600 !bg-transparent glass-effect !rounded-md"
        align="start"
      >
        <Command className="w-full border-gray-600 !bg-transparent glass-effect !rounded-md">
          ...
          <CommandList className="max-h-60 scrollbar-hide-default !bg-transparent">
            <CommandEmpty className="country-select-empty !bg-transparent">
              No country found.
            </CommandEmpty>
            <CommandGroup className="bg-transparent">
              {countriesWithFlag.map((country) => (
                ...
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4 text-sky-500",
                      value === country.alpha2 ? "opacity-100" : "opacity-0"
                    )}
                  />
                  <span className="flex items-center gap-2">
