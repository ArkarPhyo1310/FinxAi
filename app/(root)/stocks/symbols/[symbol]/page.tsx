  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-7xl mx-auto py-8">
      ...
      <section className="flex flex-col gap-6">
        <TradingViewWidget scriptUrl={`${scriptUrlBase}symbol-info.js`} config={SYMBOL_INFO_WIDGET_CONFIG(symbol)} />
        <TradingViewWidget
          scriptUrl={`${scriptUrlBase}advanced-chart.js`}
          config={CANDLE_CHART_WIDGET_CONFIG(symbol)}
        />
        <TradingViewWidget scriptUrl={`${scriptUrlBase}advanced-chart.js`} config={BASELINE_WIDGET_CONFIG(symbol)} />
      </section>
      {/* Right Column */}
      <section className="flex flex-col gap-6">
